...
describe('Actors DAO', () => {
  test('#findAll finds all actors', async (done) => {
    ...
    // when
    const result$ = ActorsMethods.findAll(defaultQuery);
  ...
  test('#findById finds actor by _id', async (done) => {
    ...
    // when
    const result$ = ActorsMethods.findById(targetUser._id);
  ...
  test('#findOneByImdbID finds actor by "imdb" identifier', async (done) => {
    ...
    // when
    const result$ = ActorsMethods.findOneByImdbID(targetUser.imdbId!);
